...
> **Status — 2026-09-07:** experimental research code. The strongest mechanism evidence is finite-budget subset allocation. Five preregistered n=30 Q-scaling blocks are complete: two Digits/MLP, one Digits/SmallCNN, FashionMNIST/Tiny Transformer, and CIFAR-10/ResNet-20. The three Digits blocks and the CIFAR block give positive low-Q versus high-Q attenuation with exact Q=K identity; Fashion gives weaker borderline cross-task support. CIFAR additionally retains a positive exploratory attenuation after excluding the forced Q=K endpoint. The downstream learned-function mediator remains unidentified.
...
| SmallCNN endpoint sensitivity, post-hoc | low mean(Q2,Q4)-Q12 **+0.476 pp**, 95% CI [-0.537,+1.490], descriptive p=0.172 | original PASS does not by itself establish attenuation among non-full Q values |
| FashionMNIST Tiny Transformer Q-scaling, Issue #73, 30 pairs | attenuation **+0.314 pp**, 95% CI [-0.0065,+0.6340], preregistered one-sided p=0.0273; Q=8 exact identity | first cross-task directional support, but borderline |
...
See [CIFAR budget result](docs/CIFAR_BUDGET_SCALING_RESULT.md), [Fashion budget result](docs/FASHION_BUDGET_SCALING_RESULT.md), [CNN budget result](docs/CNN_BUDGET_SCALING_RESULT.md), and [research status](docs/RESEARCH_STATUS.md).
...
> Under a binding subset-update budget, hard + gradient-nonredundant environment selection can outperform hardness-only allocation in several tested structured regimes. Finite-budget dependence is strongly replicated across MLP and SmallCNN within Digits/geometric and now has strong CIFAR-10/ResNet-20 cross-task support, with weaker preregistered FashionMNIST/Tiny Transformer support. In every Q-scaling test, the methods become exactly identical when all K candidates contribute. The downstream learned-function mediator remains open.
...
Do **not** read this as a universal gradient-diversity law. Pure diversity can be weak, larger accumulated gradient rank is not sufficient, and Q-response shape is task dependent.
...
The CIFAR curve is monotone in this observed block; the SmallCNN and Fashion curves are not. Universal monotonicity is therefore not claimed. The preregistered common quantity is the low-Q versus high-Q contrast plus exact Q=K identity.

At Q=K, loss-hard and gradnov have the same candidate set and update order. Exact model-state identity was verified in every paired run of all five preregistered Q-scaling blocks.
...
The original primary 40-pair Q=4 gradient-novel minus loss-hard mean difference was +0.1206 pp, raw p=0.002672 and Holm(5) p=0.01336. The new independent Q-scaling block uses fresh replicate and environment seeds and shows a preregistered low-vs-high attenuation of +0.1168 pp, p=0.003804, with Q=8 exact identity. p10/minimum results remain secondary and do not establish tail safety.
...
Additional Q-scaling replications now have lower marginal value. The next mechanistic priority is a **loss-stratified redundancy intervention**: hold Q and loss-rank structure fixed while prospectively forcing high versus low gradient non-redundancy. This can test whether non-redundancy itself contributes causally, rather than relying on Q=K disappearance.
