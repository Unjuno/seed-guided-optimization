...
TRAIN_START=71000; TEST_START=72000
...
def transform_photometric(Xb, seed, offset):
    ...
    gain=r.uniform(.78,1.22); bright=r.uniform(-.10,.10); noise=r.uniform(0,.09)
    orient=int(r.integers(0,2)); width=int(r.integers(1,4)); pos=int(r.integers(0,8)); atten=r.uniform(0,.45)
    edge=int(r.integers(0,4)); edge_width=int(r.integers(0,3)); edge_atten=r.uniform(.2,.8)
    a=Xb.astype(np.float32)*gain+bright
    if orient==0:
        y0=max(0,min(7,pos-width//2)); y1=min(8,y0+width); a[:,y0:y1,:]*=atten
    else:
        x0=max(0,min(7,pos-width//2)); x1=min(8,x0+width); a[:,:,x0:x1]*=atten
    if edge_width>0:
        if edge==0:a[:,:edge_width,:]*=edge_atten
        elif edge==1:a[:,-edge_width:,:]*=edge_atten
        elif edge==2:a[:,:,:edge_width]*=edge_atten
        else:a[:,:,-edge_width:]*=edge_atten
    rr=np.random.default_rng(seed*9001+offset)
...
if __name__=='__main__':
    ap=argparse.ArgumentParser(); ap.add_argument('mode',choices=['diagnostic','evaluate']); ap.add_argument('--start',type=int,default=0); ap.add_argument('--end',type=int,default=20); ap.add_argument('--outdir',default='/mnt/data/prospective_bands')
