...
TRAIN_START=61000; TEST_START=62000
...
def transform_photometric(Xb, seed, offset):
    ...
    sigma=r.uniform(.03,.38)
    mask_p=r.uniform(0,.30)
    gain=r.uniform(.72,1.28)
    bright=r.uniform(-.18,.18)
    salt_p=r.uniform(0,.08)
    rr=np.random.default_rng(seed*7001+offset)
    a=Xb.astype(np.float32)*gain+bright
    a=a+rr.normal(0,sigma,a.shape).astype(np.float32)
    keep=(rr.random(a.shape)>=mask_p)
    a=a*keep
    salt=rr.random(a.shape)<salt_p
    pepper=rr.random(a.shape)<salt_p
    a=np.where(salt,1.0,a); a=np.where(pepper,0.0,a)
...
if __name__=='__main__':
    ap=argparse.ArgumentParser(); ap.add_argument('mode',choices=['diagnostic','evaluate']); ap.add_argument('--start',type=int,default=0); ap.add_argument('--end',type=int,default=20); ap.add_argument('--outdir',default='/mnt/data/prospective_unstructured')
