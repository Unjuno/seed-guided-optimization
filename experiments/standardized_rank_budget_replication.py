...
HELDOUT_SEEDS = np.arange(17000, 17080, dtype=int)
...
def standardized_effective_rank(features: np.ndarray) -> tuple[float, int]:
    h = np.asarray(features, dtype=np.float64)
    mean = h.mean(axis=0, keepdims=True)
    sd = h.std(axis=0, ddof=0, keepdims=True)
    keep = sd.reshape(-1) > STD_EPS
    n_retained = int(keep.sum())
    if n_retained == 0:
        return 1.0, 0
    z = (h[:, keep] - mean[:, keep]) / sd[:, keep]
    return effective_rank_features(z), n_retained
...
def representation_diagnostics(
    ...
    raw_rank = effective_rank_features(all_features)
    standardized_rank, retained_channels = standardized_effective_rank(all_features)
    return raw_rank, standardized_rank, retained_channels
    ...
    for rep in range(start, end):
        ...
        for q in Q_VALUES:
            for method in ("loss_hard", "gradnov"):
                ...
                raw_rank, standardized_rank, retained_channels = representation_diagnostics(
                    model, train_environments, probe_idx
                )
                row = {
                    "rep": rep,
                    "method": method,
                    ...
                    "candidate_k": K,
                    "coverage_ratio": q / K,
                    "rep_eff_rank": raw_rank,
                    "standardized_rep_eff_rank": standardized_rank,
                    "standardized_rank_retained_channels": retained_channels,
        ...
        pd.DataFrame(rows).to_csv(
            output_dir / f"standardized_rank_budget_diagnostics_{start}_{end}.csv",
            index=False,
        )
        print(
...
def evaluate_shard(start: int, end: int, output_dir: Path) -> None:
    ...
    # Fresh held-out environments are intentionally constructed only here,
    # after the training-only diagnostics and model states have been sealed.
    heldout = [
    ...
    pd.DataFrame(rows).to_csv(
        output_dir / f"standardized_rank_budget_heldout_{start}_{end}.csv",
        index=False,
    )
