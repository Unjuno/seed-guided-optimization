"""Frozen Issue #76 aggregation and decision."""
...
def validate(df,label):
    ...
    if len(df)!=len(exp) or obs!=exp or df.duplicated(['rep','q','method']).any(): raise ValueError(label+' grid mismatch')
...
def main():
    ...
    tr=load(root,'cifar_budget_training_'); he=load(root,'cifar_budget_heldout_'); validate(tr,'training'); validate(he,'heldout')
    if set(tr.state_digest)!=set(he.state_digest): raise ValueError('training/heldout digest sets differ')
    ...
    if len(allr)!=240: raise ValueError('merged row count')
    ...
    decision='INVALID / Q8 IDENTITY FAILURE' if not identity else 'CIFAR RESNET FINITE-BUDGET COVERAGE REPLICATES' if coverage else 'CIFAR COVERAGE THEORY FAIL'
    qrows=[]
    for q in QS:
    ...
    allr.to_csv(out/'cifar_budget_all30.csv',index=False); de.to_csv(out/'cifar_budget_deltas30.csv',index=False); att.to_csv(out/'cifar_budget_attenuation30.csv',index=False); qs.to_csv(out/'cifar_budget_q_summary30.csv',index=False); dec.to_csv(out/'cifar_budget_decision30.csv',index=False); pd.DataFrame(mism,columns=['rep','field','loss_hard','gradnov']).to_csv(out/'cifar_budget_q8_mismatches.csv',index=False)
