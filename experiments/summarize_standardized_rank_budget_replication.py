...
EXPECTED_REPS = tuple(range(300, 330))
Q_VALUES = (2, 4, 8, 12, 16)
METHODS = ("loss_hard", "gradnov")
DIAGNOSTIC_METRICS = (
    "rep_eff_rank",
    "standardized_rep_eff_rank",
    "standardized_rank_retained_channels",
...
def q16_identity(all_rows: pd.DataFrame) -> tuple[bool, pd.DataFrame]:
    ...
    columns = ["rep", "field", "loss_hard", "gradnov"]
    return len(mismatches) == 0, pd.DataFrame(mismatches, columns=columns)
...
def main() -> None:
    ...
    diagnostics = load_parts(input_dir, "standardized_rank_budget_diagnostics_")
    heldout = load_parts(input_dir, "standardized_rank_budget_heldout_")
    ...
    for rep in EXPECTED_REPS:
        ...
        z_low = float(part.loc[[2, 4], "delta_standardized_rep_eff_rank"].mean())
        z_high = float(part.loc[[12, 16], "delta_standardized_rep_eff_rank"].mean())
        raw_low = float(part.loc[[2, 4], "delta_rep_eff_rank"].mean())
        raw_high = float(part.loc[[12, 16], "delta_rep_eff_rank"].mean())
        ...
        z = part.loc[list(Q_VALUES), "delta_standardized_rep_eff_rank"].to_numpy(dtype=np.float64)
        attenuation_rows.append(
            {
                ...
                "standardized_rank_low": z_low,
                "standardized_rank_high": z_high,
                "standardized_rank_attenuation": z_low - z_high,
                "raw_rank_low": raw_low,
                "raw_rank_high": raw_high,
                "raw_rank_attenuation": raw_low - raw_high,
                "benefit_slope_vs_coverage": float(np.polyfit(x, b, 1)[0]),
                "standardized_rank_slope_vs_coverage": float(np.polyfit(x, z, 1)[0]),
            }
        )
    ...
    t_z, p_z = one_sided_positive_ttest(
        attenuation["standardized_rank_attenuation"].to_numpy()
    )
    t_raw, p_raw = one_sided_positive_ttest(
        attenuation["raw_rank_attenuation"].to_numpy()
    )

    coverage_pass = bool(
        attenuation["benefit_attenuation"].mean() > 0 and p_b < 0.05
    )
    standardized_mediator_pass = bool(
        attenuation["standardized_rank_attenuation"].mean() > 0 and p_z < 0.05
    )
    ...
    if not identity_pass:
        ...
        decision = "COVERAGE THEORY FAIL"
    elif standardized_mediator_pass:
        decision = "SCALE-INVARIANT MEDIATOR CANDIDATE PASS"
    else:
        decision = "COVERAGE REPLICATION ONLY"

    if (
        len(attenuation) >= 3
        and attenuation["benefit_attenuation"].std(ddof=1) > 0
        and attenuation["standardized_rank_attenuation"].std(ddof=1) > 0
    ):
        corr = stats.pearsonr(
            attenuation["benefit_attenuation"],
            attenuation["standardized_rank_attenuation"],
        )
    ...
    for q in Q_VALUES:
        ...
        for metric in (
            "delta_mean_test",
            "delta_standardized_rep_eff_rank",
            "delta_rep_eff_rank",
        ...
        [
            {
                ...
                "coverage_replication_pass": coverage_pass,
                "mean_standardized_rank_low": float(
                    attenuation["standardized_rank_low"].mean()
                ),
                "mean_standardized_rank_high": float(
                    attenuation["standardized_rank_high"].mean()
                ),
                "mean_standardized_rank_attenuation": float(
                    attenuation["standardized_rank_attenuation"].mean()
                ),
                "standardized_rank_attenuation_t": t_z,
                "standardized_rank_attenuation_p_one_sided": p_z,
                "standardized_mediator_pass": standardized_mediator_pass,
                "mean_raw_rank_attenuation_secondary": float(
                    attenuation["raw_rank_attenuation"].mean()
                ),
                "raw_rank_attenuation_t_secondary": t_raw,
                "raw_rank_attenuation_p_one_sided_secondary": p_raw,
                "attenuation_pearson_r_secondary": attenuation_corr,
                "attenuation_pearson_p_secondary": attenuation_corr_p,
                "decision": decision,
            }
        ]
    )
    ...
    all_rows.to_csv(
        output_dir / "standardized_rank_budget_all30.csv", index=False
    )
    deltas.to_csv(
        output_dir / "standardized_rank_budget_deltas30.csv", index=False
    )
    attenuation.to_csv(
        output_dir / "standardized_rank_budget_attenuation30.csv", index=False
    )
    q_summary.to_csv(
        output_dir / "standardized_rank_budget_q_summary30.csv", index=False
    )
    decision_row.to_csv(
        output_dir / "standardized_rank_budget_decision30.csv", index=False
    )
    identity_mismatches.to_csv(
        output_dir / "standardized_rank_budget_q16_identity_mismatches.csv",
        index=False,
    )
