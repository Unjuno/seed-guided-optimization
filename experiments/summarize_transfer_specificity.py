...
EXPECTED_REPS=tuple(range(900,930)); METHODS=('loss_hard','gradnov'); FAMILIES=('shared','nuisance'); METRICS=('mean_test','sd_test','p10_test','min_test','clean_test')
...
def main()->None:
    ap=argparse.ArgumentParser(); ap.add_argument('--input-dir',required=True); ap.add_argument('--output-dir',required=True); ap.add_argument('--sealed-alpha',type=float,required=True); a=ap.parse_args(); root=Path(a.input_dir); out=Path(a.output_dir); out.mkdir(parents=True,exist_ok=True)
    diag=load_parts(root,'transfer_diagnostics_'); held=load_parts(root,'transfer_heldout_'); validate_diag(diag); validate_held(held)
    nuisance=held[held.family=='nuisance']; vals=nuisance.alpha.dropna().to_numpy(float)
    if len(vals)==0 or not np.all(np.isclose(vals,a.sealed_alpha)): raise ValueError('sealed alpha mismatch')
    ...
    for rep in EXPECTED_REPS:
        part=held[held.rep==rep]; row={'rep':rep}; benefits={}
        for fam in FAMILIES:
            p=part[part.family==fam].set_index('method'); famrow={'rep':rep,'family':fam}
            for metric in METRICS: famrow['delta_'+metric]=float(p.loc['gradnov',metric]-p.loc['loss_hard',metric])
            deltas.append(famrow); benefits[fam]=famrow['delta_mean_test']
    ...
    deltas=pd.DataFrame(deltas); contrasts=pd.DataFrame(contrasts); bs=contrasts.shared_benefit.to_numpy(float); sp=contrasts.specificity_contrast.to_numpy(float); ts,ps=one_sided(bs); tt,pt=one_sided(sp); shared_pass=bool(bs.mean()>0 and ps<0.05); specificity_pass=bool(sp.mean()>0 and pt<0.05)
