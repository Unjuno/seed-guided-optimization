...
EXPECTED_REPS=tuple(range(700,730))
FAMILIES=('structured','nuisance')
...
def main()->None:
    ap=argparse.ArgumentParser(); ap.add_argument('--input-dir',required=True); ap.add_argument('--output-dir',required=True); ap.add_argument('--sealed-alpha',type=float,required=True); ap.add_argument('--sealed-beta',type=float,required=True); a=ap.parse_args()
    root=Path(a.input_dir); out=Path(a.output_dir); out.mkdir(parents=True,exist_ok=True)
    diag=load_parts(root,'two_axis_diagnostics_'); held=load_parts(root,'two_axis_heldout_'); validate(diag,'diagnostics'); validate(held,'heldout')
    for frame,label in ((diag,'training'),(held,'heldout')):
        ...
        if not np.all(np.isclose(n.beta.to_numpy(float),a.sealed_beta)): raise ValueError(f'{label} beta mismatch')
    ...
    for rep in EXPECTED_REPS:
        ...
        for fam in FAMILIES:
            ...
            ng=float(t.loc['gradnov','selected_pairwise_novelty']-t.loc['loss_hard','selected_pairwise_novelty']); cl=float(t['mean_candidate_loss'].mean()); sl=float(t['mean_selected_loss'].mean())
    ...
    s=train[train.family=='structured']; n=train[train.family=='nuisance']; ns=float(s.novelty_gain.mean()); nn=float(n.novelty_gain.mean()); ls=float(s.candidate_loss_level.mean()); ln=float(n.candidate_loss_level.mean()); nm=abs(ns-nn); lm=abs(ls-ln); match=bool(nm<=0.03 and lm<=0.10)
    ...
    dec=pd.DataFrame([{'n':len(x),'sealed_alpha':a.sealed_alpha,'sealed_beta':a.sealed_beta,'mean_structured_novelty_gain':ns,'mean_nuisance_novelty_gain':nn,'novelty_mismatch':nm,'novelty_tolerance':0.03,'mean_structured_candidate_loss':ls,'mean_nuisance_candidate_loss':ln,'candidate_loss_mismatch':lm,'candidate_loss_tolerance':0.10,'match_pass':match,'mean_structured_benefit':float(conv.structured_benefit.mean()),'mean_nuisance_benefit':float(conv.nuisance_benefit.mean()),'mean_conversion_contrast':float(x.mean()),'conversion_se':se,'conversion_t':t,'conversion_p_one_sided':p,'conversion_pass':cp,'decision':decision}])
    train.to_csv(out/'two_axis_confirm_training30.csv',index=False); family_delta.to_csv(out/'two_axis_family_deltas30.csv',index=False); conv.to_csv(out/'two_axis_conversion30.csv',index=False); dec.to_csv(out/'two_axis_decision30.csv',index=False); print(dec.to_string(index=False),flush=True)
